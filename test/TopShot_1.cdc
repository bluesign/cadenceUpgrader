...
access(all) contract TopShot: NonFungibleToken {
    ...
    access(all) fun Network() : String { return "mainnet" }

    // The address to which royalties should be deposited
    access(all) fun RoyaltyAddress() : Address { return 0xfaf0cc52c6e3acaf }

    // The path to the Subedition Admin resource belonging to the Account
    // which the contract is deployed on
    access(all) fun SubeditionAdminStoragePath() : StoragePath { return /storage/TopShotSubeditionAdmin}
    ...
    access(all) resource Set {
        ...
        access(all) fun getPlays(): [UInt32] {
            return self.plays
        }

        access(all) fun getRetired(): {UInt32: Bool} {
            return self.retired
        }

        access(all) fun getNumMintedPerPlay(): {UInt32: UInt32} {
    ...
    access(all) struct QuerySetData {
        ...
        access(all) fun getNumberMintedPerPlay(): {UInt32: UInt32} {
            return self.numberMintedPerPlay
        }
    }
    ...
    access(all) resource NFT: NonFungibleToken.INFT, ViewResolver.Resolver {
            ...
            switch view {
                ...
                case Type<MetadataViews.Royalties>():
                    let royaltyReceiver: Capability<&{FungibleToken.Receiver}> =
                        getAccount(TopShot.RoyaltyAddress()).capabilities.get<&AnyResource>(MetadataViews.getRoyaltyReceiverPublicPath())!
        ...
        return <-TopShot.createEmptyCollection(nftType: Type<@Collection>())
    })
        ...
        access(all) fun getMomentURL(): String {
...
access(all) fun createEmptyCollection(): @{NonFungibleToken.Collection}{
 return <-create Collection() 
    ...
    //
    access(all) resource Collection: MomentCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection { 
        ...
        access(all) fun lock(id: UInt64, duration: UFix64) {
            // Remove the nft from the Collection
        ...
        access(all) fun batchLock(ids: [UInt64], duration: UFix64) {
        ...
        access(all) fun unlock(id: UInt64) {
            // Remove the nft from the Collection
            let token <- self.ownedNFTs.remove(key: id) 
                ?? panic("Cannot lock: Moment does not exist in the collection")
        ...
        access(all) fun batchUnlock(ids: [UInt64]) {
        ...
        access(all) fun destroyMoments(ids: [UInt64]) {
...
access(all) fun createEmptyCollection(): @{NonFungibleToken.Collection}{
 return <-create Collection() 
    ...
    //
    access(all) fun getMomentsSubedition(nftID: UInt64):UInt32? {
    ...
    access(all) fun getAllSubeditions():[TopShot.Subedition] {
        let subeditionAdmin = self.account.storage.borrow<&SubeditionAdmin>(from: TopShot.SubeditionAdminStoragePath())
            ?? panic("No subedition admin resource in storage")
    ...
    access(all) fun getSubeditionByID(subeditionID: UInt32):TopShot.Subedition {
        let subeditionAdmin = self.account.storage.borrow<&SubeditionAdmin>(from: TopShot.SubeditionAdminStoragePath())
            ?? panic("No subedition admin resource in storage")
    ...
    access(all) fun getNextSubeditionID():UInt32 {
        let subeditionAdmin = self.account.storage.borrow<&SubeditionAdmin>(from: TopShot.SubeditionAdminStoragePath())
            ?? panic("No subedition admin resource in storage")
    ...
    init() {
        ...
        var _capForLinked1 = self.account.capabilities.storage.issue<&{MomentCollectionPublic}>( /storage/MomentCollection)
self.account.capabilities.publish(_capForLinked1 , at:/public/MomentCollection)
