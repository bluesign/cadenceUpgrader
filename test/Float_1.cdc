                ...
                case Type<MetadataViews.Royalties>():
                    return MetadataViews.Royalties([
						MetadataViews.Royalty(
							receiver: getAccount(0x5643fd47a29770e7).capabilities.get<&FlowToken.Vault>(/public/flowTokenReceiver)!,
...
view fun borrowFLOAT(id: UInt64): &NFT?
    ...
    access(all) resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection, CollectionPublic {
        ...
        access(all) fun delete(id: UInt64) {
...
view fun userCanMint(address: Address): Bool
    ...
    access(all) resource FLOATEvent: FLOATEventPublic, ViewResolver.Resolver {
        ...
        // Type: Admin Toggle
        access(all) fun toggleVisibilityMode() {
...
view fun getPrices(): {String: TokenInfo}? {
    return self.extraMetadata["prices"] as! {String: TokenInfo}?
        ...
        access(all) fun mint(recipient: &Collection, optExtraFloatMetadata: {String: AnyStruct}?): UInt64 {
            pre {
...
view fun borrowPublicEventRef(eventId: UInt64): &FLOATEvent? {
    ...
    access(all) fun resolveView(_ view: Type): AnyStruct? {
        ...
        return <-FLOAT.createEmptyCollection(nftType: Type<@Collection>())
    })
            case Type<MetadataViews.NFTCollectionDisplay>():
    ...
    init() {
        ...
        if self.account.storage.borrow<&FLOAT.Collection>(from: FLOAT.FLOATCollectionStoragePath) == nil {
            self.account.storage.save(<- FLOAT.createEmptyCollection(nftType: Type<@Collection>()), to: FLOAT.FLOATCollectionStoragePath)
            var _capForLinked1 = self.account.capabilities.storage.issue<&FLOAT.Collection>( FLOAT.FLOATCollectionStoragePath)
self.account.capabilities.publish(_capForLinked1 , at:FLOAT.FLOATCollectionPublicPath)
        ...
        if self.account.storage.borrow<&FLOAT.FLOATEvents>(from: FLOAT.FLOATEventsStoragePath) == nil {
            ...
            var _capForLinked2 = self.account.capabilities.storage.issue<&FLOAT.FLOATEvents>( FLOAT.FLOATEventsStoragePath)
self.account.capabilities.publish(_capForLinked2 , at:FLOAT.FLOATEventsPublicPath)

        }

        let FLOATEvents = self.account.storage.borrow<&FLOAT.FLOATEvents>(from: FLOAT.FLOATEventsStoragePath)
