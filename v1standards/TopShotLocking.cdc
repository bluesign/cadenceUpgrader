...
contract TopShotLocking{ 
	...
	fun lockNFT(
		...
		let expiryTimestamp =
			getCurrentBlock().timestamp + duration
	...
	access(all)
	fun unlockNFT(nft: @{NonFungibleToken.NFT}): @{
		NonFungibleToken.NFT
	}{ 
		if !self.lockedNFTs.containsKey(nft.id){ 
		...
		let lockExpiryTimestamp: UFix64 =
			self.lockedNFTs[nft.id]!
	...
	init(){ 
		...
		self.account.storage.save(
			<-admin,
			to: TopShotLocking.AdminStoragePath()
		)
	}
}
