...
contract PonsNftContract{ 
	...
	access(account)
	let ArtistAuthorityStoragePath: StoragePath
	...
	access(account)
	var ponsArtists: @{String: PonsArtist}
	
	/* Stores the ponsArtistId corresponding to each Address */
	access(account)
	var ponsArtistIds:{ Address: String}
	...
	access(all)
	event PonsNftWithdrawFromCollection(nftId: String, serialNumber: UInt64, from: Address?)
	
	/* PonsNftWithdrawFromCollection is emitted on depositing of Pons NFTs to a colllection */
	access(all)
	event PonsNftDepositToCollection(nftId: String, serialNumber: UInt64, to: Address?)
	
	/* PonsArtistRecognised is emitted on the recognition an artist by Pons on the blockchain */
	access(all)
	event PonsArtistRecognised(
		ponsArtistId: String,
		metadata:{ 
			String: String
		},
	...
	access(account)
	fun emitPonsNftWithdrawFromCollection(
		nftId: String,
		serialNumber: UInt64,
		from: Address?
	): Void{ 
	...
	access(account)
	fun emitPonsNftDepositToCollection(nftId: String, serialNumber: UInt64, to: Address?): Void{ 
	...
	access(all)
	fun getNftId(_ ponsNftRef: &PonsNftContractInterface.NFT): String{ 
		return ponsNftRef.nftId
	}
	
	/* Gets the serialNumber of a Pons NFT */
	access(all)
	fun getSerialNumber(_ ponsNftRef: &PonsNftContractInterface.NFT): UInt64{ 
		return ponsNftRef.id
	}
	
	/* Borrows the PonsArtist of a Pons NFT */
	access(all)
	fun borrowArtist(_ ponsNftRef: &PonsNftContractInterface.NFT): &PonsArtist{ 
	...
	*/
	...
	access(all)
	resource interface PonsNftContractImplementation{ 
		access(all)
		fun borrowArtist(_ ponsNftRef: &PonsNftContractInterface.NFT): &PonsArtist
	...
	access(account)
	var historicalImplementations: @[{PonsNftContractImplementation}]
	
	/* The currently active instance of PonsNftContractImplementation */
	access(account)
	var implementation: @{PonsNftContractImplementation}
	
	/* Updates the currently active PonsNftContractImplementation */
	access(account)
	fun update(_ newImplementation: @{PonsNftContractImplementation}): Void{ 
	...
	access(all)
	resource InvalidPonsNftContractImplementation: PonsNftContractImplementation{ 
		access(all)
		fun borrowArtist(_ ponsNftRef: &PonsNftContractInterface.NFT): &PonsArtist{ 
	...
	*/
	...
	access(all)
	resource PonsArtistCertificate{ 
		access(all)
		let ponsArtistId: String
	...
	access(all)
	fun borrowArtistById(ponsArtistId: String): &PonsArtist{ 
	...
	*/
	...
	resource PonsArtistAuthority{ 
		...
		access(all)
		fun makePonsArtistCertificateFromArtistRef(
		...
		access(all)
		fun makePonsArtistCertificateFromId(ponsArtistId: String): @PonsArtistCertificate{ 
			return <-create PonsArtistCertificate(ponsArtistId: ponsArtistId)
		}
	}
	...
	init(){ 
		...
		self.account.storage.save(<-create PonsArtistAuthority(), to: /storage/ponsArtistAuthority)
