...
contract OlympicPinMarket{ 
	...
	access(all)
	event PieceListed(id: UInt64, price: UFix64, seller: Address?)
	
	/// emitted when the price of a listed piece has changed
	access(all)
	event PiecePriceChanged(id: UInt64, newPrice: UFix64, seller: Address?)
	
	/// emitted when a piece is purchased from the market
	access(all)
	event PiecePurchased(id: UInt64, price: UFix64, seller: Address?)
	
	/// emitted when a piece has been withdrawn from the sale
	access(all)
	event PieceCanceled(id: UInt64, owner: Address?)
	...
	resource interface SalePublic{ 
		...
		access(all)
		fun purchase(pieceId: UInt64, buyTokens: @NWayUtilityCoin.Vault): @OlympicPin.NFT{ 
			post{ 
				result.id == pieceId:
		...
		access(all)
		fun getPrice(pieceId: UInt64): UFix64?
	...
	resource SaleCollection: SalePublic{ 
		...
		access(self)
		var ownerCollection: Capability<&OlympicPin.Collection>
		...
		access(all)
		fun listForSale(pieceId: UInt64, price: UFix64){ 
			pre{ 
