...
contract CoatCheck{ 
	...
	access(all)
	event TicketRedeemed(ticketResourceID: UInt64, redeemer: Address)
	
	access(all)
	event ValetDestroyed(resourceID: UInt64)
	
	access(account)
	let valet: @CoatCheck.Valet
	
	// A CoatCheck has been initialized. This resource can now be watched for deposited items that accounts can redeem
	access(all)
	event CoatCheckInitialized(resourceID: UInt64)
	
	access(all)
	resource interface TicketPublic{ 
		access(all)
		fun redeem(
		...
		access(all)
		fun getDetails(): CoatCheck.TicketDetails
	}
	...
	access(all)
	resource Ticket: TicketPublic{ 
		// a ticket can have Fungible Tokens AND NonFungibleTokens
		access(self)
		var fungibleTokenVaults: @[{FungibleToken.Vault}]?
		
		access(self)
		var nonFungibleTokens: @[{NonFungibleToken.NFT}]?
		...
		access(self)
		var redeemed: Bool // a ticket can only be redeemed once. It also cannot be destroyed unless redeemed is set to true
		
		
		access(self)
		var storageFee: UFix64 // the storage fee taken to hold this ticket in storage. It is returned when the ticket is redeemed.
		...
		access(all)
		fun redeem(fungibleTokenReceiver: &{FungibleToken.Receiver}?, nonFungibleTokenReceiver: &{NonFungibleToken.CollectionPublic}?){ 
			pre{ 
	...
	access(all)
	resource interface ValetPublic{ 
		...
		access(all)
		fun borrowTicket(ticketID: UInt64): &Ticket?
	}
	
	access(all)
	resource Valet: ValetPublic{ 
		access(self)
		var tickets: @{UInt64: Ticket}
		
		// we store the fees taken when a ticket is made so that the exact amount is withdrawn
		// when a ticket is redeemed
		access(self)
		var feesByTicketID:{ UInt64: UFix64}
		...
		access(account)
		fun createTicket(redeemer: Address, vaults: @[{FungibleToken.Vault}]?, tokens: @[{NonFungibleToken.NFT}]?){ 
		...
		fun borrowTicket(ticketID: UInt64): &Ticket?{ 
			if self.tickets[ticketID] != nil{ 
				return &self.tickets[ticketID] as &Ticket?
			} else{ 
				return nil
			}
		}
		...
		access(all)
		fun redeemTicket(ticketID: UInt64, fungibleTokenReceiver: &{FungibleToken.Receiver}?, nonFungibleTokenReceiver: &{NonFungibleToken.CollectionPublic}?){ 
			pre{ 
	...
	access(all)
	fun getValetPublic(): &CoatCheck.Valet{ 
		return &self.valet as &CoatCheck.Valet
	}
	
	access(account)
	fun getValet(): &CoatCheck.Valet{ 
		return &self.valet as &CoatCheck.Valet
	}
