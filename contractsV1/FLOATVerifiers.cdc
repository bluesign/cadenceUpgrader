...
access(all) contract FLOATVerifiers {
    ...
    access(all) struct MinimumBalance: FLOAT.IVerifier {
        ...
        access(all) fun verify(_ params: {String: AnyStruct}) {
            ...
            let flowVault = getAccount(claimee).capabilities.get<&FlowToken.Vault>(/public/flowTokenBalance)!
                                .borrow()
    ...
    access(all) struct ChallengeAchievementPoint: FLOAT.IVerifier {
        ...
        access(all) fun verify(_ params: {String: AnyStruct}) {
            let claimee: Address = params["claimee"]! as! Address
            if let achievementBoard = getAccount(claimee).capabilities.get<&FLOATEventSeries.AchievementBoard>(FLOATEventSeries.FLOATAchievementBoardPublicPath)!
                .borrow()
            {
