...
contract FLOATVerifiers{ 
	...
	struct MinimumBalance: FLOAT.IVerifier{ 
		...
		fun verify(_ params:{ String: AnyStruct}){ 
			...
			let flowVault = (getAccount(claimee).capabilities.get(/public/flowTokenBalance)!).borrow<&FlowToken.Vault>() ?? panic("Could not borrow the Flow Token Vault")
	...
	struct ChallengeAchievementPoint: FLOAT.IVerifier{ 
		...
		fun verify(_ params:{ String: AnyStruct}){ 
			let claimee: Address = params["claimee"]! as! Address
			if let achievementBoard = (getAccount(claimee).capabilities.get(FLOATEventSeries.FLOATAchievementBoardPublicPath)!).borrow<&FLOATEventSeries.AchievementBoard>(){ 
