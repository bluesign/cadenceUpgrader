...
access(all) contract EmeraldIdentityLilico {
    ...
    access(all) resource Administrator {
        ...
        access(all) fun createAdministrator(): Capability<&Administrator> {
            return EmeraldIdentityLilico.account.capabilities.get<&Administrator>(EmeraldIdentityLilico.AdministratorPrivatePath)!
        }
...
view fun getAccountFromDiscord(discordID: String): Address? {
    ...
    init() {
        ...
        (fun():Capability?{
var cap = self.account.capabilities.storage.issue<&Administrator>( EmeraldIdentityLilico.AdministratorStoragePath)
self.account.capabilities.publish(cap , at:EmeraldIdentityLilico.AdministratorPrivatePath)
return cap
})()
    }
}