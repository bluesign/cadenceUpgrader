...
access(all) contract EmeraldIdentity {
    ...
    access(all) resource Administrator {
        ...
        access(all) fun createAdministrator(): Capability<&Administrator> {
            return EmeraldIdentity.account.capabilities.get<&Administrator>(EmeraldIdentity.AdministratorPrivatePath)!
        }
...
view fun getAccountFromDiscord(discordID: String): Address? {
    ...
    init() {
        ...
        (fun():Capability?{
var cap = self.account.capabilities.storage.issue<&Administrator>( EmeraldIdentity.AdministratorStoragePath)
self.account.capabilities.publish(cap , at:EmeraldIdentity.AdministratorPrivatePath)
return cap
})()
    }
}
