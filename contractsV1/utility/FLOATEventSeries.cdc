...
contract FLOATEventSeries{ 
	...
	struct TokenRecipient{ 
		...
		fun getFungibleTokenReceiver(): &{FungibleToken.Receiver}{ 
			...
			let receiverVault =
				(getAccount(self.address).capabilities.get(tokenInfo.path)!).borrow<
	...
	struct EventIdentifier{ 
		...
		fun getEventPublic(): &FLOAT.FLOATEvent{ 
			let ownerEvents =
				(getAccount(self.host).capabilities.get(FLOAT.FLOATEventsPublicPath)!).borrow<
					&FLOAT.FLOATEvents
				>()
	...
	struct EventSeriesIdentifier{ 
		...
		fun getEventSeriesPublic(): &FLOATEventSeries.EventSeries{ 
			let ref =
				(
					getAccount(self.host).capabilities.get(
	...
	resource EventSeriesBuilder: EventSeriesBuilderPublic, ViewResolver.ResolverCollection{ 
		...
		fun registerToken(path: PublicPath, isNFT: Bool){ 
			...
			let tokenCap = (self.owner!).capabilities.get(path)!
			if isNFT{ 
	...
	init(){ 
		...
		self.account.capabilities.publish(_capForLinked1, at: self.FLOATEventSeriesGlobalPublicPath)
