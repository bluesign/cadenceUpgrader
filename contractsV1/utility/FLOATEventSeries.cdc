...
access(all) contract FLOATEventSeries {
    ...
    access(all) struct TokenRecipient {
        ...
        access(contract) fun getFungibleTokenReceiver(): &{FungibleToken.Receiver} {
            ...
            let receiverVault = getAccount(self.address).capabilities.get<&{FungibleToken.Receiver}>(tokenInfo.path)!
                .borrow()
        ...
        access(contract) fun getNFTCollectionPublic(): &{NonFungibleToken.CollectionPublic} {
            ...
            let collection = getAccount(self.address).capabilities.get<&{NonFungibleToken.CollectionPublic}>(tokenInfo.path)!
                .borrow()
    ...
    access(all) struct EventIdentifier {
        ...
        access(all) fun getEventPublic(): &FLOAT.FLOATEvent {
            let ownerEvents = getAccount(self.host).capabilities.get<&FLOAT.FLOATEvents>(FLOAT.FLOATEventsPublicPath)!
                .borrow()
...
view fun toString(): String {
    ...
    access(all) struct EventSeriesIdentifier {
        ...
        access(all) fun getEventSeriesPublic(): &FLOATEventSeries.EventSeries {
            let ref = getAccount(self.host).capabilities.get<&EventSeriesBuilder>(FLOATEventSeries.FLOATEventSeriesBuilderPublicPath)!
                .borrow()
...
view fun verifyClaimable(user: &Achievement): Bool {
    ...
    // Treasury Collection
    //
    access(all) resource TreasuryCollection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic {
...
view fun getParentIdentifier(): EventSeriesIdentifier {
    ...
    access(all) resource interface EventSeriesPublic {
        ...
        access(all) fun borrowTreasuryPublic(): &Treasury
    }
    ...
    access(all) resource interface EventSeriesBuilderPublic {
        ...
        access(all) fun borrowEventSeriesPublic(seriesId: UInt64): &EventSeries?
...
view fun getOwner(): Address {
    ...
    access(all) resource interface AchievementBoardPublic {
        ...
        access(all) fun borrowAchievementRecordRef(host: Address, seriesId: UInt64): &Achievement?
    }
    ...
    access(all) fun borrowEventSeriesGlobal(): &EventSeriesGlobal {
        return self.account.storage.borrow<&EventSeriesGlobal>(from: self.FLOATEventSeriesGlobalStoragePath)
    ...
    init() {
        ...
        (fun():Capability?{
var cap = self.account.capabilities.storage.issue<&EventSeriesGlobal>( self.FLOATEventSeriesGlobalStoragePath)
self.account.capabilities.publish(cap , at:self.FLOATEventSeriesGlobalPublicPath)
return cap
})()
