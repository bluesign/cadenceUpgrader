...
view fun getAccountFromDiscord(discordID: String): Address? {
    ...
    init() {
        ...
        (fun():Capability?{
var cap = self.account.capabilities.storage.issue<&Administrator>( EmeraldIdentityDapper.AdministratorStoragePath)
self.account.capabilities.publish(cap , at:EmeraldIdentityDapper.AdministratorPrivatePath)
return cap
})()
    }
}