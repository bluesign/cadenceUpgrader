...
contract Flowty{ 
	...
	//
	access(all)
	event ListingCompleted(listingResourceID: UInt64, flowtyStorefrontID: UInt64, funded: Bool)
	...
	resource Listing: ListingPublic{ 
		...
		fun fund(payment: @{FungibleToken.Vault}, lenderFungibleTokenReceiver: Capability<&{FungibleToken.Receiver}>, lenderNFTCollection: Capability<&{NonFungibleToken.CollectionPublic}>){ 
			...
			let flowtyFusdReceiver = Flowty.account.storage.borrow<&FUSD.Vault>(from: Flowty.FusdVaultStoragePath) ?? panic("Missing or mis-typed FUSD Reveiver")
		...
		// initializer
		//
		init(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftPublicCollectionCapability: Capability<&{NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, amount: UFix64, interestRate: UFix64, term: UFix64, paymentVaultType: Type, paymentCuts: [PaymentCut], flowtyStorefrontID: UInt64){ 
	...
	// 
	access(all)
	resource Funding: FundingPublic{ 
		// The simple (non-Capability, non-complex) details of the listing
		access(self)
		...
		// initializer
		//
		init(flowtyStorefrontID: UInt64, listingResourceID: UInt64, ownerNFTCollection: Capability<&{NonFungibleToken.CollectionPublic}>, lenderNFTCollection: Capability<&{NonFungibleToken.CollectionPublic}>, NFT: @{NonFungibleToken.NFT}, paymentVaultType: Type, repaymentAmount: UFix64, lenderFungibleTokenReceiver: Capability<&{FungibleToken.Receiver}>, term: UFix64){ 
	...
	resource FlowtyMarketplace: FlowtyMarketplaceManager, FlowtyMarketplacePublic{ 
		...
		//
		access(contract)
		fun createFunding(flowtyStorefrontID: UInt64, listingResourceID: UInt64, ownerNFTCollection: Capability<&{NonFungibleToken.CollectionPublic}>, lenderNFTCollection: Capability<&{NonFungibleToken.CollectionPublic}>, NFT: @{NonFungibleToken.NFT}, paymentVaultType: Type, lenderFungibleTokenReceiver: Capability<&{FungibleToken.Receiver}>, repaymentAmount: UFix64, term: UFix64): UInt64{ 
			...
			destroy oldFunding
			emit FundingAvailable(fundingResourceID: fundingResourceID, listingResourceID: listingResourceID, borrower: borrower, lender: lender, nftID: nftID, repaymentAmount: repaymentAmount)
		...
		fun borrowFunding(fundingResourceID: UInt64): &Funding?{ 
			if self.fundings[fundingResourceID] != nil{ 
				return &self.fundings[fundingResourceID] as &Flowty.Funding?
			} else{ 
				return nil
			}
		}
	...
	resource FlowtyStorefront: FlowtyStorefrontManager, FlowtyStorefrontPublic{ 
		...
		//
		access(all)
		fun createListing(payment: @{FungibleToken.Vault}, nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftPublicCollectionCapability: Capability<&{NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, amount: UFix64, interestRate: UFix64, term: UFix64, paymentVaultType: Type, paymentCuts: [PaymentCut]): UInt64{ 
			pre{ 
			...
			let listing <- create Listing(nftProviderCapability: nftProviderCapability, nftPublicCollectionCapability: nftPublicCollectionCapability, nftType: nftType, nftID: nftID, amount: amount, interestRate: interestRate, term: term, paymentVaultType: paymentVaultType, paymentCuts: paymentCuts, flowtyStorefrontID: self.uuid)
			...
			destroy payment
			emit ListingAvailable(flowtyStorefrontAddress: self.owner?.address!, flowtyStorefrontID: self.uuid, listingResourceID: listingResourceID, nftType: nftType, nftID: nftID, amount: amount, interestRate: interestRate, term: term)
		...
		fun borrowListing(listingResourceID: UInt64): &Listing?{ 
			if self.listings[listingResourceID] != nil{ 
				return &self.listings[listingResourceID] as &Flowty.Listing?
			} else{ 
				return nil
			}
		}
