...
view fun getPrices(): {String: TokenInfo}? {
    return self.extraMetadata["prices"] as! {String: TokenInfo}?
        ...
        access(all) fun purchase(recipient: &Collection, params: {String: AnyStruct}, payment: @{FungibleToken.Vault}) {
            ...
            let EventHostVault = getAccount(self.host).capabilities.get<&{FungibleToken.Receiver}>(tokenInfo.path)!
                                    .borrow()
            ...
            let EmeraldCityVault = getAccount(emeraldCityTreasury).capabilities.get<&{FungibleToken.Receiver}>(tokenInfo.path)!
                                    .borrow() 
...
view fun borrowPublicEventRef(eventId: UInt64): &FLOATEvent? {
    ...
    init() {
        ...
        if self.account.storage.borrow<&FLOAT.Collection>(from: FLOAT.FLOATCollectionStoragePath) == nil {
            ...
            (fun():Capability?{
var cap = self.account.capabilities.storage.issue<&FLOAT.Collection>( FLOAT.FLOATCollectionStoragePath)
self.account.capabilities.publish(cap , at:FLOAT.FLOATCollectionPublicPath)
return cap
})()
        }
        ...
        if self.account.storage.borrow<&FLOAT.FLOATEvents>(from: FLOAT.FLOATEventsStoragePath) == nil {
            ...
            (fun():Capability?{
var cap = self.account.capabilities.storage.issue<&FLOAT.FLOATEvents>( FLOAT.FLOATEventsStoragePath)
self.account.capabilities.publish(cap , at:FLOAT.FLOATEventsPublicPath)
return cap
})()
        }
