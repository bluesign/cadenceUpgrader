...
contract FLOAT: NonFungibleToken, ViewResolver{ 
	...
	resource interface FLOATEventPublic{ 
		...
		access(all)
		fun getExtraFloatMetadata(serial: UInt64):{ String: AnyStruct}
	...
	resource FLOATEvent: FLOATEventPublic, ViewResolver.Resolver{ 
		...
		fun purchase(recipient: &Collection, params:{ String: AnyStruct}, payment: @{FungibleToken.Vault}){ 
			...
			let EventHostVault = (getAccount(self.host).capabilities.get(tokenInfo.path)!).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow the &{FungibleToken.Receiver} from the event host.")
			...
			let EmeraldCityVault = (getAccount(emeraldCityTreasury).capabilities.get(tokenInfo.path)!).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow the &{FungibleToken.Receiver} from Emerald City's Vault.")
