...
pub contract MindtrixUtility {
  ...
  // Metadata
  //========================================================
  access(account) fun generateMetadataHash(strMetadata: {String: String}): [UInt8] {  
    var metadataString = "{"
    // The field order matters. It affects the hash result.
    var attributes: [String] = ["Background", "Body", "Color", "Face", "Gloves", "Headgear", "Offhand", "Serial No"]
  ...
  pub fun upperCaseFirstChar(_ str: String): String {    
    return self.toUpperCase(str.slice(from: 0, upTo: 1)).concat(str.slice(from: 1, upTo: str.length))
  }
