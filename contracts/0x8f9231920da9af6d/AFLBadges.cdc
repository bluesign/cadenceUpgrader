...
pub contract AFLBadges {
    ...
    pub event BadgeCreated(id: UInt64, name: String, description: String, imageUrl: String)
    pub event BadgeUpdated(id: UInt64, name: String, description: String, imageUrl: String)
    ...
    pub resource Admin {
         ...
         **/
        pub fun createBadge(name: String, description: String, imageUrl: String): UInt64 {
            let badge = Badge(id: AFLBadges.nextBadgeId, name: name, description: description, imageUrl: imageUrl)
            AFLBadges.badges[AFLBadges.nextBadgeId] = badge
            AFLBadges.nextBadgeId = AFLBadges.nextBadgeId + 1
            emit BadgeCreated(id: badge.id, name: badge.name, description: badge.description, imageUrl: badge.imageUrl)
        ...
        **/        
        pub fun updateBadge(id: UInt64, name: String, description: String, imageUrl: String) {
            let badge = Badge(id: id, name: name, description: description, imageUrl: imageUrl)
            AFLBadges.badges[id] = badge
            emit BadgeUpdated(id: badge.id, name: badge.name, description: badge.description, imageUrl: badge.imageUrl)
        }
        ...
        **/
        pub fun addBadgeToTemplate(badgeId: UInt64, templateId: UInt64) {
            pre {
                AFLBadges.badges[badgeId] != nil: "No badge for badge id: ".concat(badgeId.toString())
            }
        ...
        **/
        pub fun removeBadgeFromTemplate(badgeId: UInt64, templateId: UInt64) {
            pre {
                AFLBadges.badges[badgeId] != nil: "No badge for badge id"
                AFLBadges.badgeIdsByTemplateId[templateId] != nil: "No badges for template id"
                AFLBadges.badgeIdsByTemplateId[templateId]!.contains(badgeId): "Badge does not exist for template id"
            }
            AFLBadges.badgeIdsByTemplateId[templateId]!.remove(at: AFLBadges.badgeIdsByTemplateId[templateId]!.firstIndex(of: badgeId)!)
        ...
        * @param templateId The ID of the template to remove the badge from.
        **/
        pub fun removeBadgeFromTemplates(badgeId: UInt64, templateIds: [UInt64]) {
            for id in templateIds {
