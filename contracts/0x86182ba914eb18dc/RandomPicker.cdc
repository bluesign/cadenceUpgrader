pub contract RandomPicker {
    ...
    pub resource SelectionDetails {
        ...
        access(contract) fun getUniqueRandomString(startRange: UInt64, endRange: UInt64): String {
        ...
        access(contract) fun selectString(): SelectedWinner {
            if(self.isNumbers){
                if (self.allowMultipleSelections) {
                    ...
                    if(   self.winners.length == Int(self.endRange - self.startRange)){panic( "No strings left to select.")}
                    ...
                    if(   self.strings.length == 0){panic( "No strings left to select.")}
        ...
        init(strings: [String], allowMultipleSelections: Bool,startRange:UInt64,endRange:UInt64,isNumbers:Bool) {
    ...
    pub fun borrowAdmin(): &Admin {
        return self.account.borrow<&RandomPicker.Admin>(from: self.AdminStoragePath)!
    }
    ...
    init() {
        ...
        self.account.link<&RandomPicker.Admin{RandomPicker.IAdminPublic}>(self.AdminPublicPath, target: self.AdminStoragePath)
    }
}
