access(all) contract FlowBlocksTradingScore {
    // -----------------------------------------------------------------------
    // Contract Events
    // -----------------------------------------------------------------------
    access(all) event ContractInitialized()
    access(all) event TradingScoreIncreased(wallet: Address, points: UInt32)
    access(all) event TradingScoreDecreased(wallet: Address, points: UInt32)
    ...
    // Contract Fields
    // -----------------------------------------------------------------------
    access(self) var tradingScores: {Address: UInt32}

    access(all) resource Admin {

        access(all) fun deductPoints(wallet: Address, pointsToDeduct: UInt32) {
            pre {
                FlowBlocksTradingScore.tradingScores[wallet] != nil: "Can't deduct points: Address has no trading score."
            }
    ...
    access(all) fun getTradingScores(): {Address: UInt32} {
        return FlowBlocksTradingScore.tradingScores
    }

    access(all) fun getTradingScore(wallet: Address): UInt32? {
    ...
    init() {
        ...
        self.account.link<&FlowBlocksTradingScore.Admin>(self.AdminPrivatePath, target: self.AdminStoragePath) 
