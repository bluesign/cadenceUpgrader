access(all) contract Veriphoto {

    access(all) event ImageHashRecordCreated(creator: Address)
    access(all) event ImageRegistered(imageHash: String, owner: Address, metadata: ImageMetadata)

    access(all) let imageHashRecordStoragePath: StoragePath
    access(all) let imageHashRecordMetadataReaderPath: PrivatePath
    access(all) let imageHashRecordVerifierPath: PublicPath

    access(all) let accountRegistry: { Address: Bool }

    access(all) var oracleAddress: Address

    access(all) struct ImageMetadata {
        access(all) let imageTimestamp: UFix64
        access(all) let extras: { String: String }?
        access(all) var blockTimestamp: UFix64?
    ...
    access(all) resource interface Verifier {
        access(all) fun verifyImage(imageHash: String): Bool
    }

    access(all) resource interface MetadataReader {
        access(all) fun getImageMetadata(imageHash: String): ImageMetadata?
    }

    access(all) resource ImageHashRecord: Verifier, MetadataReader {
        access(all) let creator: Address
        access(self) let imageMetadata: { String: ImageMetadata }

        // question: make imageHash and hexSignature [UInt8] and rename hexSig to just signature ?
        access(all) fun registerImage(imageHash: String, hexSignature: String, metadata: ImageMetadata) {
            pre {
                self.owner!.address == self.creator: "collection is not owned by creator"
                Veriphoto.accountRegistry[self.creator]! == true: "account has been invalidated"
                metadata.blockTimestamp == nil: "metadata.blockTimestamp has already been set"
            }

            // verify image signature
            let creatorKey: PublicKey = getAccount(self.creator).keys.get(keyIndex: 0)!.publicKey
            let isSignatureValid = creatorKey.verify(
                signature: hexSignature.decodeHex(),
                signedData: imageHash.utf8,
                domainSeparationTag: "",
        ...
        access(all) fun deregisterImage(imageHash: String) {
            pre {
                self.owner!.address == self.creator: "collection is not owned by creator"
                Veriphoto.accountRegistry[self.creator]! == true: "account has been invalidated"
            }
    ...
    access(all) fun createEmptyImageHashRecord(creator: Address, oracleHexSignature: String, ownerHexSignature: String): @ImageHashRecord {
        pre {
            !self.accountRegistry.containsKey(creator): "account already registered: ".concat(creator.toString())
        }

        // verify oracle signature
        let oracleKey: PublicKey = getAccount(self.oracleAddress).keys.get(keyIndex: 0)!.publicKey
        let isVerificationSignatureValid = oracleKey.verify(
    ...
    access(account) fun updateAccountStatus(creator: Address, isValid: Bool) {
        pre {
            self.accountRegistry.containsKey(creator): "account not registered: ".concat(creator.toString())
        }
